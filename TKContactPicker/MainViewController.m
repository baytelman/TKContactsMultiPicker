...
- (BOOL)tkPeoplePickerController:(TKContactsMultiPickerController*)picker shouldIncludeContact:(TKContact*)contact alreadyIncluded:(NSArray *)contacts
{
    for (TKContact * existing in contacts) {
        if ([contact.email isEqualToString:existing.email])
            return NO;
    }
